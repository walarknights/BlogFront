...
app3.get(`/followerList/:id`, async (c) => {
  ...
  try {
    ...
    for (let i = 0; i < followerIds.length; i++) {
      const followerRow = await db.prepare("SELECT id, username, avatar FROM users WHERE id = ?").bind(followerIds[i]).first();
      if (!followerRow) {
      ...
      if (followerRow) {
        ...
        if (filePath) {
          followerRow.avatar = await generatePresignedUrl3(filePath, c.env);
        }
        followers.push({
...
// .wrangler/tmp/bundle-Tuzo8f/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-Tuzo8f/middleware-loader.entry.ts
