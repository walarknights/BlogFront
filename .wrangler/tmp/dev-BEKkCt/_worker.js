...
app3.get(`/followList/:id`, async (c) => {
  ...
  const db = c.env.DB;
  try {
    const sql = `SELECT followeeId FROM follows WHERE followerId = ?`;
    const result = await db.prepare(sql).bind(userId).all();
    console.log("followList result:", result);
...
// .wrangler/tmp/bundle-uXAiNR/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-uXAiNR/middleware-loader.entry.ts
