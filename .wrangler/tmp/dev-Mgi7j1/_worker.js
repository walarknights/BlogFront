...
var JwtPayloadRequiresAud = class extends Error {
  ...
  constructor(payload2) {
    super(`required "aud" in jwt payload: ${JSON.stringify(payload2)}`);
...
var sign = /* @__PURE__ */ __name(async (payload2, privateKey, alg = "HS256") => {
  const encodedPayload = encodeJwtPart(payload2);
  let encodedHeader;
...
var verify = /* @__PURE__ */ __name(async (token, publicKey, algOrOptions) => {
  ...
  const { header, payload: payload2 } = decode(token);
  if (!isTokenHeader(header)) {
    throw new JwtHeaderInvalid(header);
  }
  const now = Date.now() / 1e3 | 0;
  if (nbf && payload2.nbf && payload2.nbf > now) {
    throw new JwtTokenNotBefore(token);
  }
  if (exp && payload2.exp && payload2.exp <= now) {
    throw new JwtTokenExpired(token);
  }
  if (iat && payload2.iat && now < payload2.iat) {
    throw new JwtTokenIssuedAt(now, payload2.iat);
  }
  if (iss) {
    if (!payload2.iss) {
      throw new JwtTokenIssuer(iss, null);
    }
    if (typeof iss === "string" && payload2.iss !== iss) {
      throw new JwtTokenIssuer(iss, payload2.iss);
    }
    if (iss instanceof RegExp && !iss.test(payload2.iss)) {
      throw new JwtTokenIssuer(iss, payload2.iss);
    }
  }
  if (aud) {
    if (!payload2.aud) {
      throw new JwtPayloadRequiresAud(payload2);
    }
    const audiences = Array.isArray(payload2.aud) ? payload2.aud : [payload2.aud];
    const matched = audiences.some(
      (payloadAud) => aud instanceof RegExp ? aud.test(payloadAud) : typeof aud === "string" ? payloadAud === aud : Array.isArray(aud) && aud.includes(payloadAud)
    );
    if (!matched) {
      throw new JwtTokenAudience(aud, payload2.aud);
    }
  }
...
var decode = /* @__PURE__ */ __name((token) => {
  try {
    ...
    const payload2 = decodeJwtPart(p);
    return {
      header,
      payload: payload2
    };
...
app.get("/getInfo", async (c) => {
  const db = c.env.DB;
  const payload2 = await c.req.json();
  const results = await db.prepare(
    "SELECT id, username, followers, followings, avatar, dynamicNum FROM users WHERE id = ?"
  ).bind(payload2.id).first();
  return c.json(results ?? []);
});
app.post("/login", async (c) => {
  const payload2 = await c.req.json();
  if (!payload2?.username || !payload2?.password) {
    return c.json({ error: "username and password are required" }, 400);
  }
  const db = c.env.DB;
  const result = await db.prepare("SELECT password FROM users WHERE username = ?").bind(payload2.username).first();
  if (!result) {
    return c.json({ message: "\u4E0D\u5B58\u5728\u8BE5\u7528\u6237" }, 401);
  }
  const hashedPassword = result.password;
  if (!await checkPassword(payload2.password, hashedPassword)) {
    return c.json({ message: "\u5BC6\u7801\u9519\u8BEF" }, 401);
  }
  const userInfo = await db.prepare(
    "SELECT id, username, followers, followings, avatar, dynamicNum FROM users WHERE username = ?"
  ).bind(payload2.username).first();
  const user = {
    id: userInfo.id,
  ...
  return c.json({ message: "\u767B\u5F55\u6210\u529F", user, token });
});
app.post("/setUser", async (c) => {
  const res = await stmt.bind(payload.username, payload.password, defaultAvatar).run();
  return c.json({ success: true, lastRowId: res.meta?.last_row_id });
});
app.get("/:id", async (c) => {
...
app.post(`/personal/:id/setting `, async (c) => {
  ...
  if (body.file instanceof File) {
    ...
    const uploadUrl = `https://${c.env.B2_ENDPOINT}/file/${c.env.B2_BUCKET_NAME}/${filePath}`;
...
app.post(`personal/:id/addFollow`, async (c) => {
  const payload2 = await c.req.json();
  const followerId = payload2.follower;
  const followeeId = payload2.followee;
...
var app2 = new Hono2();
app2.get("/getList", async (c) => {
  let list = [];
  const sql = `SELECT id, title, content, authorId, createdAt, abstract, coverUrl FROM article ORDER BY createdAt DESC LIMIT 4`;
  const db = c.env.DB;
  try {
    const result = await db.prepare(sql).all();
    list = result.results;
    return c.json({ articles: list ?? [], message: "success" });
  } catch (error) {
    return c.json({ articles: [], message: "error", error });
  }
});
...
app3.post("/upload", async (c) => {
  ...
  return c.json({ message: "false load" });
});
...
app4.use(
  ...
  cors({
    ...
    allowHeaders: ["Content-Length", "Content-Type"],
    // 关键点:放开或回显
...
// .wrangler/tmp/bundle-XkLnfl/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-XkLnfl/middleware-loader.entry.ts
