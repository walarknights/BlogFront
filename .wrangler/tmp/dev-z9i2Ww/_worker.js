...
app2.post("/history/view", async (c) => {
  try {
...
app2.get("/getHistory/:userId", async (c) => {
...
app3.post("/addHistory/:id", async (c) => {
  const payload = await c.req.json();
  if (!payload.userId || !payload.articleId) {
  ...
  try {
    await db.prepare("INSERT INTO history (userId, articleId) VALUES (?, ?)").bind(userId, articleId).run();
...
app3.get("/getHistory/:id", async (c) => {
  ...
  const res = await db.prepare("SELECT articleId FROM history WHERE userId = ? ORDER BY timestamp DESC").bind(userId).all();
  const articleIds = res.results.map((row) => row.articleId);
  const articles = [];
  for (const articleId of articleIds) {
    const article = await db.prepare("SELECT id, title, content FROM articles WHERE id = ?").bind(articleId).first();
    let coverUrl = "";
    if (article && article.coverUrl && typeof article.coverUrl === "string") {
      coverUrl = article.coverUrl ? article.coverUrl : "";
      article.coverUrl = await generatePresignedUrl3(coverUrl, c.env);
...
// .wrangler/tmp/bundle-1K1rjY/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-1K1rjY/middleware-loader.entry.ts
