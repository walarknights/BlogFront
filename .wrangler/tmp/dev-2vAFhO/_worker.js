...
app2.post("/", async (c) => {
  try {
    ...
    const db = c.env.DB;
    const user = await db.prepare("SELECT id FROM users WHERE id = ?").bind(body.userId).first();
    if (!user) {
      return c.json({ message: "\u7528\u6237Id\u9519\u8BEF" }, 401);
    }
    await db.prepare(
      "INSERT INTO article (userId, content, title, type, abstract, link) VALUES (?, ?, ?, ?, ?, ?)"
    ).bind(body.userId, body.content, body.title, body.type, body.abstract, body.link || "").run();
...
app2.get("/:id", async (c) => {
  ...
  try {
    ...
    let coverUrl = "";
    const cover = await db.prepare("SELECT url FROM articleimg WHERE Id = ? LIMIT 1").bind(id).first();
    if (cover) {
      coverUrl = cover.url;
...
app2.post("/cover", async (c) => {
  try {
    ...
    const fileName = `article_cover_${Date.now()}_${file.name}`;
    const filePath = `article/cover/${fileName}`;
    const uploadUrl = `https://${c.env.B2_ENDPOINT}/${c.env.B2_BUCKET_NAME}/${filePath}`;
    const uploadResponse = await fetch(uploadUrl, {
      method: "PUT",
      headers: {
    ...
    const db = c.env.DB;
    const result = await db.prepare("INSERT INTO articleimg (url) VALUES (?)").bind(filePath).run();
    return c.json({
      message: "\u4E0A\u4F20\u6210\u529F",
      filename: fileName,
      url: filePath,
    ...
    console.error("SaveCover error:", error);
...
app2.get("/list", async (c) => {
  ...
  try {
    const { results } = await db.prepare(
      "SELECT id, userId, content, title, type, abstract, likes, favorites, link FROM article ORDER BY id DESC LIMIT 10"
    ).all();
    const articles = [];
...
// .wrangler/tmp/bundle-CZEnBt/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-CZEnBt/middleware-loader.entry.ts
