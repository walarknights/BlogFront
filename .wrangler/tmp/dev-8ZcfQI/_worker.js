...
app2.post("/search", async (c) => {
  try {
    ...
    for (const article of results) {
      try {
        article.coverUrl = await generatePresignedUrl2(article.coverUrl, c.env);
      } catch (error) {
...
app2.post("/like/:id", async (c) => {
  ...
  try {
    ...
    if (result) {
      ...
      await db.prepare("UPDATE article SET likes = likes + ? WHERE id = ?").bind(change, articleId).run();
    }
...
app2.post(`/removeLike/:id`, async (c) => {
  const articleId = c.req.param("id");
  const { userId } = await c.req.json();
  const db = c.env.DB;
  try {
    ...
    if (res.success) {
      const res2 = await db.prepare("UPDATE article SET likes = likes - 1 WHERE id = ?").bind(articleId).run();
      if (!res2.success) {
...
app2.post("/favorite/:id", async (c) => {
  ...
  try {
    ...
    if (res.success) {
      const res2 = await db.prepare("UPDATE article SET favorites = favorites + 1 WHERE id = ?").bind(articleId).run();
      if (!res2.success) {
...
app2.post("/removeFavorite/:id", async (c) => {
  const articleId = c.req.param("id");
  const { userId } = await c.req.json();
  const db = c.env.DB;
  try {
...
app2.post("/isFavorite/:id", async (c) => {
  const articleId = c.req.param("id");
  const { userId } = await c.req.json();
  const db = c.env.DB;
  try {
    ...
    const comments = await Promise.all(
      results.map(async (row) => {
        ...
        if (userAvatar && !userAvatar.startsWith("http")) {
          userAvatar = await generatePresignedUrl2(userAvatar, c.env);
        }
        let parentUserAvatar = row.parent_user_avatar;
        if (parentUserAvatar && !parentUserAvatar.startsWith("http")) {
          parentUserAvatar = await generatePresignedUrl2(parentUserAvatar, c.env);
        }
        const comment = {
          commentId: row.id,
          content: row.content,
...
app2.post("/addComment/:id", async (c) => {
  try {
    const input = await c.req.json();
    const db = c.env.DB;
    const createdAt = (/* @__PURE__ */ new Date()).toISOString();
    const userInfo = await db.prepare("SELECT username, avatar FROM users WHERE id = ?").bind(input.userId).first();
    if (!userInfo) {
...
// src/server/personal/index.ts
var app3 = new Hono2();
var generatePresignedUrl3 = /* @__PURE__ */ __name(async (path, env) => {
  if (!path) return "";
  if (path.startsWith("http")) return path;
  try {
    const aws = new AwsClient({
...
var hashPassword2 = /* @__PURE__ */ __name(async (password) => {
  const encoder2 = new TextEncoder();
  const data = encoder2.encode(password);
  const hashBuffer = await crypto.subtle.digest("SHA-256", data);
  const hashArray = Array.from(new Uint8Array(hashBuffer));
...
var formatDateTime = /* @__PURE__ */ __name((isoString) => {
  return isoString.replace("T", " ").replace(/\.\d{3}Z$/, "");
}, "formatDateTime");
app3.get("/getInfo/:id", async (c) => {
  ...
  row.avatar = await generatePresignedUrl3(row.avatar, c.env);
  if (!row) {
    return c.json({ error: "User not found" }, 404);
  }
  return c.json(row);
});
app3.get("/getPersonalArticle/:id", async (c) => {
  const userId = c.req.param("id");
  if (!userId) {
    return c.json({ error: "request is invalid" }, 401);
  }
  const db = c.env.DB;
  const res = await db.prepare("SELECT id FROM article WHERE userId = ? LIMIT 10").bind(userId).all();
  const articleIds = res.results.map((row) => row.id);
  const articles = [];
  for (const articleId of articleIds) {
    const article = await db.prepare("SELECT id, title, content, coverUrl FROM article WHERE id = ?").bind(articleId).first();
    let coverUrl = "";
    if (article && article.coverUrl && typeof article.coverUrl === "string") {
      coverUrl = article.coverUrl ? article.coverUrl : "";
      article.coverUrl = await generatePresignedUrl3(coverUrl, c.env);
    }
    if (article) {
      articles.push({
        id: article.id,
  ...
  try {
    ...
    if (res.success === true) {
      try {
        ...
        if (row.avatar) {
          userInfo.avatar = await generatePresignedUrl3(row.avatar, c.env);
        }
        if (row.password) {
          userInfo.password = await hashPassword2(row.password);
        }
        userInfo.id = row.id;
        userInfo.username = row.username;
        userInfo.followers = row.followers;
        userInfo.followings = row.followings;
        userInfo.dynamicNum = row.dynamicNum;
      } catch (error) {
...
app3.post(`/addFollow/:id`, async (c) => {
  const payload = await c.req.json();
  ...
  const db = c.env.DB;
  try {
    const existing = await db.prepare("SELECT 1 FROM followes WHERE followerId = ? AND followeeId = ?").bind(followerId, followeeId).first();
    if (existing) {
    ...
    if (res.success === true) {
      const res1 = await db.prepare("UPDATE users SET followings = followings + 1 WHERE id = ?").bind(followerId).run();
      const res2 = await db.prepare("UPDATE users SET followers = followers + 1 WHERE id = ?").bind(followeeId).run();
...
app3.post(`/removeFollow/:id`, async (c) => {
  const payload = await c.req.json();
  const followerId = payload.follower;
  const followeeId = payload.followee;
  if (followerId === followeeId) {
  ...
  try {
    ...
    const res = await db.prepare(sql).bind(followerId, followeeId).run();
    if (res.success === true) {
      const res1 = await db.prepare("UPDATE users SET followings = followings - 1 WHERE id = ? AND followings > 0").bind(followerId).run();
      const res2 = await db.prepare("UPDATE users SET followers = followers - 1 WHERE id = ? AND followers > 0").bind(followeeId).run();
      if (res1.success !== true || res2.success !== true) {
...
app3.get(`/followList/:id`, async (c) => {
  const userId = c.req.param("id");
  if (!userId) {
  ...
  try {
    ...
    for (let i = 0; i < followeeIds.length; i++) {
      const followeeRow = await db.prepare("SELECT id, username, avatar FROM users WHERE id = ?").bind(followeeIds[i]).first();
      if (followeeRow) {
        let filePath = "";
        if (followeeRow.avatar && typeof followeeRow.avatar === "string") {
          filePath = followeeRow.avatar;
        }
        followeeRow.avatar = await generatePresignedUrl3(filePath, c.env);
        followees.push({
          userId: followeeRow.id,
          userName: followeeRow.username,
          avatar: followeeRow.avatar,
          isFollow: true
        });
      } else {
...
app3.get(`/followerList/:id`, async (c) => {
  const userId = c.req.param("id");
  if (!userId) {
    return c.json({ error: "Missing userId parameter" }, 400);
  }
  try {
    ...
    const followerRes = await db.prepare("SELECT followerId FROM followes WHERE followeeId = ?").bind(userId).all();
    const followerIds = followerRes.results.map((row) => row.followerId);
    const followers = [];
    const followeeRes = await db.prepare("SELECT followeeId FROM followes WHERE followerId = ?").bind(userId).all();
    const followeeIds = followeeRes.results.map((row) => row.followeeId);
    for (const id of followerIds) {
      const followerRow = await db.prepare("SELECT id, username, avatar FROM users WHERE id = ?").bind(id).first();
      if (!followerRow) {
        return c.json({ error: "Failed to fetch follower user data" }, 502);
      }
      if (followerRow) {
        followerRow.avatar = await generatePresignedUrl3(followerRow.avatar, c.env);
        followers.push({
          userId: followerRow.id,
          userName: followerRow.username,
          avatar: followerRow.avatar,
          isFollow: followeeIds.includes(followerRow.id)
        });
      }
    }
...
app3.get("/getFavoritesArticles/:id", async (c) => {
  const userId = c.req.param("id");
  if (!userId) {
  ...
  const res = await db.prepare("SELECT articleId FROM favorites WHERE userId = ? AND favorites = 1").bind(userId).all();
  const articleIds = res.results.map((row) => row.articleId);
  const articles = [];
  for (const articleId of articleIds) {
    const article = await db.prepare("SELECT id, title, content FROM article WHERE id = ?").bind(articleId).first();
    let coverUrl = "";
    if (article && article.coverUrl && typeof article.coverUrl === "string") {
      coverUrl = article.coverUrl ? article.coverUrl : "";
      article.coverUrl = await generatePresignedUrl3(coverUrl, c.env);
...
app3.get("/getLikeArticles/:id", async (c) => {
  const userId = c.req.param("id");
  if (!userId) {
    return c.json({ error: "Missing userId parameter" }, 401);
  }
  ...
  const res = await db.prepare("SELECT articleId FROM likes WHERE userId = ? AND likes = 1").bind(userId).all();
  const articleIds = res.results.map((row) => row.articleId);
  const articles = [];
  for (const articleId of articleIds) {
    const article = await db.prepare("SELECT id, title, content, coverUrl FROM article WHERE id = ?").bind(articleId).first();
    ...
    if (article && article.coverUrl && typeof article.coverUrl === "string") {
      coverUrl = article.coverUrl ? article.coverUrl : "";
      coverUrl = await generatePresignedUrl3(coverUrl, c.env);
      article.coverUrl = coverUrl;
    }
    if (article) {
      articles.push({
...
app3.post("/addHistory", async (c) => {
  ...
  try {
    ...
    ).bind(userId, articleId, viewTime).run();
...
app3.get("/getHistory/:id", async (c) => {
  const userId = c.req.param("id");
  if (!userId) {
    return c.json({ error: "Missing userId parameter" }, 401);
  }
  ...
  const res = await db.prepare("SELECT articleId FROM history WHERE userId = ? ORDER BY time DESC").bind(userId).all();
  const articleIds = res.results.map((row) => row.articleId);
  const articles = [];
  for (const articleId of articleIds) {
    const article = await db.prepare("SELECT id, title, content, coverUrl FROM article WHERE id = ?").bind(articleId).first();
...
app3.post();
var personal_default = app3;

// src/server/test/index.ts
var app4 = new Hono2();
app4.get("/list", (c) => c.json({ message: "User API is working!" }));
...
app5.route("/user", users_default);
app5.route("/article", article_default);
app5.route("/personal", personal_default);
app5.route("/test", test_default);
var worker_default = {
  fetch: /* @__PURE__ */ __name((req, env, ctx) => app5.fetch(req, env, ctx), "fetch")
};
...
// .wrangler/tmp/bundle-aWfKDY/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-aWfKDY/middleware-loader.entry.ts
