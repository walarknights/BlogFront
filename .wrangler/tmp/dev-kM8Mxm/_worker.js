...
app3.get("/getPersonalArticle/:id", async (c) => {
  ...
  const db = c.env.DB;
  const res = await db.prepare("SELECT articleId FROM articles WHERE userId = ? LIMIT 10").bind(userId).all();
...
// .wrangler/tmp/bundle-nTTQtC/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-nTTQtC/middleware-loader.entry.ts
