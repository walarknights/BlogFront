...
app.post("/login", async (c) => {
  ...
  const row = await db.prepare(
    "SELECT id, username, followers, followings, avatar, dynamicNum FROM users WHERE username = ? AND password = ?"
  ).bind(payload.username, payload.password).first();
  if (!row) {
...
app.post("/setUser", async (c) => {
  ...
  const db = c.env.DB;
  const defaultAvatar = `https://${c.env.B2_BUCKET_NAME}.${c.env.B2_ENDPOINT}/usersInfo/avatar/user_0.png`;
  const stmt = db.prepare(
    "INSERT INTO users (username, password, followers, followings, avatar, dynamicNum) VALUES (?, ?, 0, 0, ?, 0)"
  );
...
// .wrangler/tmp/bundle-xlnbXc/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-xlnbXc/middleware-loader.entry.ts
