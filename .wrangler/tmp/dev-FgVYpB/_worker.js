...
app.post("/login", async (c) => {
  try {
    ...
    const avatar = `https://${c.env.B2_ENDPOINT}/${c.env.B2_BUCKET_NAME}` + userInfo.avatar;
    console.log(c.env.B2_BUCKET_NAME);
...
app3.post("/upload", async (c) => {
  ...
  try {
    ...
    if (body.file instanceof File) {
      ...
      const uploadUrl = `https://${c.env.B2_ENDPOINT}/${c.env.B2_BUCKET_NAME}/${c.env.B2_APPLICATION_KEY}/${filePath}`;
...
// .wrangler/tmp/bundle-9dtvOy/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-9dtvOy/middleware-loader.entry.ts
